...
const EditUserPage = () => {
  ...
  const getRatingsData = () => {
    ...
    return [
      { id: 4, name: 'Masterpiece', value: userStatDetails.masterpiece, emoji: '🏆' },
      { id: 3, name: 'Amazing',     value: userStatDetails.amazing,     emoji: '😍' },
      { id: 2, name: 'One Time',    value: userStatDetails.one_time,    emoji: '😐' },
      { id: 1, name: 'Unbearable',  value: userStatDetails.unbearable,  emoji: '🤮' },
    ];
  };
  ...
  const handleAvatarUpload = async () => {
    ...
    const { data, error } = await supabase.storage.from('AvatarBucket').upload(filePath, selectedFile);
    if (error) throw error;
    const { data: { publicUrl } } = supabase.storage.from('AvatarBucket').getPublicUrl(filePath);
    return publicUrl;
  };
  ...
  const deleteOldAvatar = async (oldUrl) => {
    ...
    if (!filePath) return;
    const { error } = await supabase.storage.from('AvatarBucket').remove([filePath]);
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      if (selectedFile) {
        if (userDetails.avatar_url) await deleteOldAvatar(userDetails.avatar_url);
        avatarUrl = await handleAvatarUpload();
      }
      const { error } = await supabase
        .from('user')
        .update({ name: displayName, gender: gender, avatar_url: avatarUrl })
  ...
  if (loading) {
    return (
      <div className="edit-user-page">
        <UserHeader />
        <div className="eup-loading-container"><p>Loading profile...</p></div>
      </div>
    );
  }
    ...
    <div className="edit-user-page">
      ...
      <main className="eup-main-content">
          ...
          <div className="eup-hero-content">
                ...
                >
                  {showEditForm && (
                    <>
                      ...
                      <input type="file" id="avatar-input" accept="image/*" onChange={handleFileSelect} style={{ display: 'none' }} />
                    </>
                  )}
                </div>
              </div>
            ...
            <div className="eup-user-info">
              ...
              <div className="eup-user-meta">
                ...
                <div className="eup-rank-badge-inline" onClick={() => navigate("/leaderboard")} style={{ cursor: "pointer" }}>
        ...
        <section className="eup-content-grid">
          ...
          {/* RIGHT — reviews, recommendations, edit form stacked */}
          <div className="eup-right-col">
            ...
            {showEditForm && (
              <div className="eup-edit-panel">
                ...
                <div className="eup-form-container">
                  <div className="eup-form-group">
                    ...
                    <div className="eup-input-wrapper">
                      <input type="text" value={displayName} onChange={(e) => setDisplayName(e.target.value)} placeholder="Enter full name" className="eup-glass-input" />
                  ...
                  <div className="eup-form-group">
                    ...
                    <div className="eup-input-wrapper">
                      <input type="email" value={userDetails?.email} readOnly disabled className="eup-glass-input eup-locked-input" />
                  ...
                  <div className="eup-form-group">
                    ...
                    <div className="eup-input-wrapper">
                      <select value={gender} onChange={(e) => setGender(e.target.value)} className="eup-glass-input">
                ...
                <div className="eup-action-buttons">
                  <button className="eup-btn-primary" onClick={handleSaveChanges} disabled={saving}>
