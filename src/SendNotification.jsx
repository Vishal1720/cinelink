...
const SendNotification = () => {
    ...
    useEffect(() => {
        if (!targetAllUsers && userSearchQuery.length >= 2) {
            const delayDebounceFn = setTimeout(() => {
                searchUsers();
            }, 300);
            return () => clearTimeout(delayDebounceFn);
    ...
    }, [userSearchQuery, targetAllUsers]);
    ...
    const handleToggleAllUsers = (checked) => {
        setTargetAllUsers(checked);
        if (checked) {
            setSelectedUsers([]);
            setUserSearchQuery('');
            setShowUserResults(false);
    ...
    const handleSendNotification = async (e) => {
        ...
        if (!targetAllUsers && selectedUsers.length === 0) {
            alert('Please select at least one user or enable "All Users"');
            return;
        }
        ...
        try {
            ...
            if (targetAllUsers) {
                ...
                // Use selected users
                targetUsers = selectedUsers.map(u => ({ email: u.email }));
            }
            ...
            alert(`✓ Notification sent successfully to ${targetUsers.length} user${targetUsers.length > 1 ? 's' : ''}!`);
            fetchRecentNotifications();
        } catch (error) {
                ...
                <div className="send-notif-grid">
                            ...
                            <form onSubmit={handleSendNotification} className="send-notif-form">
                                ...
                                <div className="send-notif-audience-toggle">
                                    <div>
                                        ...
                                        <p className="send-notif-audience-desc">
                                            {targetAllUsers ? 'Sending to all registered users' : `Selected ${selectedUsers.length} user${selectedUsers.length !== 1 ? 's' : ''}`}
                                        </p>
                                    </div>
                                ...
                                <div className="send-notif-fields">
                                    {/* User Selection (only when All Users is unchecked) */}
                                    {!targetAllUsers && (
                                        <div className="send-notif-field">
                                            <label className="send-notif-label">Select Users</label>
                                            <div className="send-notif-search-wrapper">
                                                <span className="material-symbols-outlined send-notif-search-icon">person_search</span>
                                                <input
                                                    type="text"
                                                    ...
                                                    onFocus={() => userSearchQuery && setShowUserResults(true)}
                                                />
                                                {userSearchLoading && (
                                            ...
                                            {showUserResults && userResults.length > 0 && (
                                                <div className="send-notif-results">
                                                    {userResults.map((user) => (
                                                        <div
                                                            key={user.email}
                                                            className="send-notif-user-result-item"
                                                            onClick={() => handleUserSelect(user)}
                                                        >
                                                            <img
                                                                src={user.avatar_url || 'https://wiggitkoxqislzddubuk.supabase.co/storage/v1/object/public/AvatarBucket/defaultavatar.jpg'}
                                                                alt={user.name}
                                                                className="send-notif-user-avatar"
                                                            />
                                                            <div className="send-notif-result-info">
                                                                <p className="send-notif-result-title">{user.name}</p>
                                                                <p className="send-notif-result-meta">{user.email}</p>
                                                            </div>
                                                        </div>
                                                    ))}
                                                </div>
                                            ...
                                            {selectedUsers.length > 0 && (
                                                <div className="send-notif-selected-users">
                                                    {selectedUsers.map((user) => (
                                                        <div key={user.email} className="send-notif-user-chip">
                                                            <img
                                                                src={user.avatar_url || 'https://wiggitkoxqislzddubuk.supabase.co/storage/v1/object/public/AvatarBucket/defaultavatar.jpg'}
                                                                alt={user.name}
                                                                className="send-notif-chip-avatar"
                                                            />
                                                            <span className="send-notif-chip-name">{user.name}</span>
                                                            <button
                                                                type="button"
                                                                className="send-notif-chip-remove"
                    ...
                    <section className="send-notif-sidebar">
                        ...
                        <div className="send-notif-stats">
                            <div className="send-notif-stat-card send-notif-stat-card-full">
